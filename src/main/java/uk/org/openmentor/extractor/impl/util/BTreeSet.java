...
package uk.org.openmentor.extractor.impl.util;
 ...
 * accordance with the specifications of the Set interface. Any attempt to add
 * a null element will result in an IllegalArgumentException being thrown. The
...
public class BTreeSet extends AbstractSet implements Set {
    ...
    private int compare(Object x, Object y) {
        return (comparator == null ? ((Comparable) x).compareTo(y) : comparator
                .compare(x, y));
    }
    ...
    private class Iterator implements java.util.Iterator {
        ...
        // Contains all parentIndicies for currentNode
        ...
        private Object nextElement() {
            if (currentNode.isLeaf()) {
                if (index < currentNode.nrElements) {
                    ...
                    // All elements have been returned, return successor of
                    // lastReturned if it exists
                    ...
                    // Reached root and he has no more children
                    return currentNode.entries[index++].element;
                }
                ...
                // Your not a leaf so simply find and return the successor
                // of lastReturned
    ...
    public class BTreeNode {
        ...
        boolean insert(Object x, int parentIndex) {
            if (isFull()) {
                // If full, you must split and promote splitNode before
                // inserting
                ...
                if (isRoot()) { // Grow a level
                    ...
                    if (BTreeSet.this.compare(x,
                            BTreeSet.this.root.entries[0].element) < 0) {
                        insert(x, 0);
                    } else {
                        rightSibling.insert(x, 1);
                    }
                } else { // Promote splitNode
                    parent.insertSplitNode(splitNode, this, rightSibling,
                            parentIndex);
                    if (BTreeSet.this.compare(x,
                            parent.entries[parentIndex].element) < 0) {
                ...
                // If not full and not leaf recursively find correct node
                // to insert at
                int insertAt = childToInsertAt(x, true);
                return (insertAt == -1 ? false : entries[insertAt].child
                        .insert(x, insertAt));
            }
            return false;
        }
        ...
        boolean delete(Object x, int parentIndex) {
            ...
            // Now temp contains element to delete and temp's parentIndex
            // is parentIndex
            ...
            if (temp.isLeaf()) {
                ...
                if (temp.nrElements > MIN) {
                    ...
                    // If leaf and have less than MIN elements, than prepare
                ...
                // Only delete at leaf so first switch with successor
                // then delete
        ...
        private BTreeNode split() {
            ...
            // Need to correct for copying the last Entry which has a null
            // element and a child
            rightSibling.nrElements--;
            return rightSibling;
        }
        ...
        private void insertSplitNode(Object splitNode, BTreeNode left,
                BTreeNode right, int insertAt) {
        ...
        private int childToInsertAt(Object x, boolean position) {
            ...
            return (!position ? hi : BTreeSet.this.compare(x,
                    entries[hi].element) == 0 ? -1 : hi);
        }
        ...
        private void prepareForDeletion(int parentIndex) {
            if (isRoot()) {
                // Don't attempt to steal or merge if your the root
                return;
                ...
                return;
                    ...
                    && parent.entries[parentIndex + 1] != null
                ...
                // If not root and can't steal left or right then try to merge
                // left
                mergeLeft(parentIndex);
                return;
            } else {
                // If not root and can't steal left or right and can't merge
                // left you must be able to merge right
        ...
        private void stealLeft(int parentIndex) {
            ...
            if (isLeaf()) {
                ...
                int add = childToInsertAt(p.entries[parentIndex - 1].element,
                        true);
